...
public class ValidCodeUtils {
	...
	public static boolean getImage(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 设定字体
		img.setFont(new Font("Times New Roman", Font.PLAIN, 64));
		...
		for (int i = 0; i < 4; i++) {
			...
			img.drawString(rand, 26 * i + 14, 36);
		}
		request.getSession().removeAttribute(RANDOMCODEKEY);
		request.getSession().setAttribute(RANDOMCODEKEY, codeValue);
	...
	public static Font getRandomFont() {
		...
		int fontSize = (int) Math.round(Math.random() * 4 + 24);
