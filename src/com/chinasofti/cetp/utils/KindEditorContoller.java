...
@Scope("prototype")
@Controller
public class KindEditorContoller {
	 ...
	 */
	@RequestMapping("doEditorUploadFile.do")
	public void doEditorUploadFile(HttpServletRequest request,
		...
		String path = "/kindEditor/" + yyyy + "/" + mm + "/upload/";
		...
		extMap.put(
				"file",
				"pdf,doc,docx,xls,xlsx,ppt,htm,html,txt,zip,rar,gz,bz2,java,php,exe,gif,jpg,jpeg,png,bmp,swf,flv,mp3,wav,wma,wmv,mid,avi,mpg,asf,rm,rmvb");
	 ...
	 * @throws IOException
	 */
	@RequestMapping("fileManagerJson.do")
	public void managerFile(HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		PrintWriter out = response.getWriter();
		...
		sdf = new SimpleDateFormat("MM");
		String mm = sdf.format(new Date());
		String temp = "/kindEditor/" + yyyy + "/" + mm + "/upload/";
		// 根目录路径，可以指定绝对路径，比如 /var/www/attached/
		String rootPath = request.getSession().getServletContext()
				.getRealPath(temp)
				+ "/";
		logger.info("rootPath:" + rootPath);
		// rootPath = fileUploadHelper.getSavePath() + temp;
		// logger.info("rootPath2:" + rootPath);
		// 根目录URL，可以指定绝对路径，比如 http://www.yoursite.com/upload/
		String rootUrl = request.getContextPath() + temp;
		// 图片扩展名
