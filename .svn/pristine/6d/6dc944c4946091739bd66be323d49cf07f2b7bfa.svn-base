 /**
 * @description This plugin allows you to make a select box editable like a text
 *              box while keeping it's select-option features
 ...
 * @example
 * 
 * var selectBox = $("select").editableSelect(); selectBox.addOption("I am
 * dynamically added");
 */
var hiInputSelect = $("<input/>");
(function($) {
	...
	$.fn.editableSelect = function() {
		...
		this.each(function() {
			...
			if (originalSelect[0].tagName.toUpperCase() === "SELECT") {
				...
				// create the selectable list that will appear when the input
				// gets focus
				...
				originalSelect.children().each(function(index, value) {
					prepareOption($(value).val(),$(value).text(), wrapper);
				});
		...
		instanceVar.addOption = function(value) {
			prepareOption("-1",value, instanceVar.parent());
		};
		
		instanceVar.getValue = function() {
			return hiInputSelect.val();
		};
		 ...
		 * 
		 * @param {String,
		 *            Number} value - the value or the index to delete
		 * @returns {void}
		 */
		instanceVar.removeOption = function(value) {
			switch (typeof (value)) {
			case "number":
				instanceVar.parent().children("ol").children(
						":nth(" + value + ")").remove();
				break;
			case "string":
				instanceVar.parent().children("ol").children().each(
						function(index, optionValue) {
		...
		instanceVar.change(function(){
			var inputText = instanceVar.val();
			var originalSelect = instanceVar.parent().children("select").children("option");
				var flag =true;
			$.each(originalSelect,function(){
				var selectText = $(this).text();
				if(inputText == selectText){		
				flag = false;
				}	
			});
				if(flag){
			hiInputSelect.val(inputText);
			}
	});
	...
	function prepareOption(value,text, wrapper) {
		var selectOption = $("<li value="+value+">" + text + "</li>").appendTo(
				wrapper.children("ol"));
		...
		selectOption.click(function() {
			hiInputSelect.val(selectOption.val());
